...
const KEYS = {
  ...
  QA_SESSIONS: "@evendi/qa_sessions",
};
...
export async function getQaSessions(): Promise<QaSession[]> {
  try {
    const data = await AsyncStorage.getItem(KEYS.QA_SESSIONS);
    return data ? JSON.parse(data) : [];
  } catch {
    return [];
  }
...
export async function saveQaSessions(sessions: QaSession[]): Promise<void> {
  await AsyncStorage.setItem(KEYS.QA_SESSIONS, JSON.stringify(sessions));
}
