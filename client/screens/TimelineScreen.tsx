...
type TimelineIconKey = keyof typeof ICON_MAP;

const normalizeTimelineIcon = (icon: string | null | undefined): TimelineIconKey =>
  icon && icon in ICON_MAP ? (icon as TimelineIconKey) : "star";

type BookedVendorResponseItem = {
  id?: string | null;
...
export default function TimelineScreen() {
  ...
  const [problematicIntervals, setProblematicIntervals] = useState<
    ScheduleInterval[]
  >([]);
  const [showOptimizationTips, setShowOptimizationTips] = useState(false);
  const [editingInterval, setEditingInterval] =
    useState<ScheduleInterval | null>(null);
  const [newTime, setNewTime] = useState("");
  const [isSaving, setIsSaving] = useState(false);
  const [showAddModal, setShowAddModal] = useState(false);
  const [newTitle, setNewTitle] = useState("");
  const [newIcon, setNewIcon] = useState<TimelineIconKey>("star");
  const [newEventTime, setNewEventTime] = useState("");
  const [eventSearch, setEventSearch] = useState("");
  const iconKeys = useMemo(
    () => Object.keys(ICON_MAP) as TimelineIconKey[],
    [],
  );

  // Weather/Location bridge state
  const [eventWeatherMap, setEventWeatherMap] = useState<
    Map<string, EventWithWeather>
  >(new Map());
  const [showWeatherOverlay, setShowWeatherOverlay] = useState(false);
  const [venueWeatherSummary, setVenueWeatherSummary] = useState<{
    avgTemp: number;
    maxPrecip: number;
    maxWind: number;
  } | null>(null);
  ...
  const { venueCoordinates, calculateBatchTravel } = locationIntel;
    ...
    try {
      ...
      if (session?.token) {
        ...
        fetchedSchedule = serverSchedule.map((event) => ({
          ...event,
          icon: normalizeTimelineIcon(event.icon),
        }));
        fetchedWeddingDate = coupleProfile.weddingDate || "";
      }
      const sortedSchedule = fetchedSchedule.sort((a, b) =>
      ...
      const errorMsg =
        err instanceof Error ? err.message : "Kunne ikke laste timelinedata";
      setError(errorMsg);
      setSchedule([]);
  ...
  const loadWeatherData = useCallback(async () => {
    try {
      ...
      const weddingDateStr = weddingDate
        ...
        : undefined;
      const eventWeather = await getEventWeather(
        session.coupleId,
        weddingDateStr,
      );
      ...
      try {
        ...
        if (sessionRaw) {
          ...
          if (vendorsRes.ok) {
            const vendorData = (await vendorsRes.json()) as unknown;
            const vendorArray = Array.isArray(vendorData)
              ? (vendorData as BookedVendorResponseItem[])
              : [];
            const vendorList = vendorArray
              .map((v) => ({
                id: v.id ?? v.vendorId ?? "",
            ...
            if (venueCoordinates && vendorList.length > 0) {
              calculateBatchTravel(
                vendorList.filter((vendor) => !!vendor.location),
              );
            }
          }
        }
  ...
  }, [weddingDate, calculateBatchTravel, venueCoordinates]);
  ...
  const handleCreateEvent = async () => {
    ...
    try {
      ...
      await import("@/lib/api-schedule-events").then(
        ({ createScheduleEvent }) =>
          createScheduleEvent(session.token!, {
            time: newEventTime,
            title: newTitle.trim(),
            icon: newIcon,
          }),
        ...
        { text: "Avbryt", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            if (!session?.token) {
              Alert.alert(
                "Innlogging kreves",
                "Logg inn for å slette hendelser.",
              );
              return;
            }
            try {
              ...
              await import("@/lib/api-schedule-events").then(
                ({ deleteScheduleEvent }) =>
                  deleteScheduleEvent(session.token!, id),
              );
  ...
  const getProblematicIntervals = useCallback((): ScheduleInterval[] => {
    const problems: ScheduleInterval[] = [];
    for (let i = 0; i < schedule.length - 1; i++) {
      const diff = getTimeDiff(schedule[i].time, schedule[i + 1].time);
      if (diff < 30) {
        problems.push({
          id: `${schedule[i].id}-${schedule[i + 1].id}`,
  ...
  const liveProblematicIntervals = useMemo(
    () => getProblematicIntervals(),
    [getProblematicIntervals],
  );
  const intervalsForUi =
    liveProblematicIntervals.length > 0
      ? liveProblematicIntervals
      : problematicIntervals;

  useEffect(() => {
    if (intervalsForUi.length > 0) {
      setShowOptimizationTips(true);
      return;
    }
    setShowOptimizationTips(false);
  }, [intervalsForUi.length]);

  const getTraditionInsight = useCallback(
    (eventTitle: string): string => {
      const insights: Record<string, Record<string, string>> = {
        norwegian: {
          Kransekake:
            "Norsk tradisjon: Kransekaken krever tid for oppstabling. Gi minst 15-20 min.",
          Brudevalsen:
            "Brudevalsen er hjertepunktet. Planlegg 10-15 min for forberedelse og gjestenes deltagelse.",
          Felemusikk:
            "Hardingfelen setter stemningen. Planlegg 5-10 min før og etter inngang.",
        },
        hindu: {
          Saptapadi:
            "De syv skritt er hellige. Planlegg 15-20 min for denne sentrale seremonien.",
          Sindoor:
            "Sindoor-seremonien er kort men betydningsfull. 5 min er nok.",
          Sangeet: "Sangeet-festen med dans og musikk. Gi 60-90 min.",
          Mehndi: "Mehndi-kveld for bruden. Planlegg 2-3 timer.",
        },
        sikh: {
          Laavan:
            "De fire runder er kjernen. Planlegg 20-30 min med hymner og ritualer.",
          Milni:
            "Familiene møtes formelt. 10-15 min for presentasjoner og girlander.",
        ...
        arabisk: {
          ...
          Kosha:
            "Brudeparet sitter på kosha. Planlegg 30 min for bilder og mottakelse.",
        },
        somalisk: {
        ...
        nigeriansk: {
          "Tradisjonell vigsel":
            "Tradisjonell vigsel med stammeeldste. Planlegg 60-90 min.",
        ...
        kinesisk: {
          Teseremoni:
            "Teseremonien for begge familier. Planlegg 30-45 min totalt.",
          "Door Games": "Door Games for brudgommen. Gi 20-30 min.",
          Bankett:
            "Kinesisk bryllupsbankett med mange retter. Planlegg 2-3 timer.",
        },
        koreansk: {
          Pyebaek:
            "Pyebaek-seremonien med bukk og kastanjer. Planlegg 20-30 min.",
          Hanbok: "Skifte til hanbok. Gi 15-20 min.",
        },
        thai: {
          "Khan Maak": "Khan Maak-prosesjonen. Gi 20-30 min.",
          Munkevelsignelse:
            "Munkene velsigner paret. Planlegg 30-45 min tidlig morgen.",
        },
        iransk: {
          "Sofreh Aghd":
            "Sofreh Aghd-oppsettet er detaljert. Planlegg 45-60 min for seremoni.",
  ...
  const filteredSchedule = useMemo(() => {
    ...
    if (!needle) return schedule;
    return schedule.filter((event) => {
      const insight = getTraditionInsight(event.title).toLowerCase();
      return `${event.time} ${event.title} ${insight}`
        .toLowerCase()
        .includes(needle);
    });
  }, [eventSearch, schedule, getTraditionInsight]);
  ...
  const handleSaveTime = async () => {
    ...
    if (!isValidTimeFormat(newTime)) {
      Alert.alert(
        "Ugyldig tid",
        "Bruk format HH:MM (f.eks. 14:30). Timer: 00-23, minutter: 00-59",
      );
      return;
    }
    ...
    try {
      ...
      const optimistic = schedule
        .map((e) =>
          e.id === editingInterval.eventIdTo ? { ...e, time: newTime } : e,
        )
        .sort((a, b) => a.time.localeCompare(b.time));
      setSchedule(optimistic);

      try {
        await updateScheduleEvent(session.token, editingInterval.eventIdTo, {
          time: newTime,
        });
        await loadData();
      } catch (err) {
  ...
  if (error) {
    return (
      ...
      >
        ...
        <Button style={{ marginTop: Spacing.lg }} onPress={loadData}>
          Prøv igjen
        </Button>
      </View>
    );
    ...
    >
      <Animated.View entering={FadeInDown.delay(100).duration(400)}>
        ...
        >
          ...
          <ThemedText
            style={[styles.headerDate, { color: theme.textSecondary }]}
          >
          ...
          {venueWeatherSummary && (
            ...
            >
              ...
              <ThemedText
                style={[styles.weatherSummaryText, { color: theme.text }]}
              >
                {Math.round(venueWeatherSummary.avgTemp)}°C •{" "}
                {venueWeatherSummary.maxPrecip > 0
                  ? `${venueWeatherSummary.maxPrecip}mm nedbør`
                  : "Tørt"}
                {venueWeatherSummary.maxWind > 8
                  ? ` • ${Math.round(venueWeatherSummary.maxWind)} m/s`
                  : ""}
              </ThemedText>
              {venueName ? (
                <ThemedText
                  style={[styles.weatherVenue, { color: theme.textSecondary }]}
                >
                  📍 {venueName}
                </ThemedText>
              ) : null}
              <EvendiIcon
                name={showWeatherOverlay ? "chevron-up" : "chevron-down"}
                size={14}
                color={theme.textSecondary}
              />
            </Pressable>
          ...
          {showWeatherOverlay && (
            <View style={styles.weatherOverlayContent}>
                  ...
                  {travelCities
                    ...
                    .map((city, i) => (
                      <View
                        key={i}
                        style={[
                          styles.travelCompactRow,
                          { borderColor: theme.border },
                        ]}
                      >
                        <ThemedText
                          style={[
                            styles.travelCompactCity,
                            { color: theme.text },
                          ]}
                        >
                          {city.name}
                        ...
                        <ThemedText
                          style={[
                            styles.travelCompactTime,
                            { color: Colors.dark.accent },
                          ]}
                        >
                          {city.drivingFormatted}
                        </ThemedText>
                        <ThemedText
                          style={[
                            styles.travelCompactDist,
                            { color: theme.textSecondary },
                          ]}
                        >
                          {city.roadDistanceKm} km
                        </ThemedText>
                      </View>
                    ))}
                </View>
                    ...
                    return (
                      ...
                      >
                        ...
                        <ThemedText style={{ fontSize: 14 }}>
                          {weatherSymbolToEmoji(ew.weather.symbol)}
                        </ThemedText>
                        <ThemedText
                          style={[
                            styles.travelCompactTime,
              ...
              {bookedVendors.length > 0 &&
                bookedVendors.some((v) =>
                  locationIntel.getTravelBadge(v.id),
                ) && (
                  <View style={styles.weatherPerEvent}>
                    <ThemedText
                      style={[styles.travelCompactTitle, { color: "#2196F3" }]}
                    >
                        ...
                        >
                          ...
                          {travel?.travel?.fuelCostNok ? (
                            ...
                            >
                              ~
                              {formatCurrency(
                                Math.round(travel.travel.fuelCostNok),
                                getSetting,
                              )}
                            </ThemedText>
                          ) : null}
      ...
      >
        ...
        <TextInput
          ...
          placeholder="Søk i kjøreplan, tid eller tradisjonstips"
      ...
      <View style={styles.timeline}>
        {filteredSchedule.map((event, index) => {
          const nextEvent = filteredSchedule[index + 1];
          const timeDiff = nextEvent
            ? getTimeDiff(event.time, nextEvent.time)
            : 0;
            ...
            >
              <View style={styles.timelineItem}>
                <View style={styles.timeColumn}>
                  <ThemedText
                    style={[styles.time, { color: Colors.dark.accent }]}
                  >
                    {event.time}
                  </ThemedText>
                </View>
                ...
                <View style={styles.dotColumn}>
                  ...
                  >
                    <EvendiIcon
                      name={ICON_MAP[event.icon] || "circle"}
                      size={12}
                      color="#1A1A1A"
                    />
                  </View>
                ...
                >
                  ...
                  {eventWeatherMap.has(event.id) &&
                    eventWeatherMap.get(event.id)?.weather && (
                      <View style={styles.eventWeatherBadge}>
                        <ThemedText style={{ fontSize: 14 }}>
                          {weatherSymbolToEmoji(
                            eventWeatherMap.get(event.id)?.weather?.symbol,
                          )}
                        </ThemedText>
                        <ThemedText
                          style={[
                            styles.eventWeatherTemp,
                            { color: theme.textSecondary },
                          ]}
                        >
                          {Math.round(
                            eventWeatherMap.get(event.id)!.weather!.temperature,
                          )}
                          °
                        </ThemedText>
                        {(eventWeatherMap.get(event.id)?.weather
                  ...
                  {eventWeatherMap.get(event.id)?.weatherTip &&
                    !eventWeatherMap
                      .get(event.id)
                      ?.weatherTip?.startsWith("✅") && (
                      <ThemedText
                        style={[
                          styles.eventWeatherTip,
                          { color: Colors.dark.accent },
                        ]}
                      >
                        {eventWeatherMap.get(event.id)?.weatherTip}
              ...
              {bufferStatus && timeDiff < 30 ? (
                <View style={styles.bufferWarning}>
                  ...
                  <Pressable
                    onPress={() =>
                      setExpandedBuffer(
                        expandedBuffer === event.id ? null : event.id,
                      )
                    }
                    style={[
                      styles.bufferCard,
                      {
                        ...
                        borderColor: bufferStatus.color,
                      },
                    ]}
                  >
                    <EvendiIcon
                      name={bufferStatus.icon}
                      size={14}
      ...
      {speeches.length > 0 && (
        ...
        >
          ...
          >
            ...
            <ThemedText
              style={[styles.sectionSubtext, { color: theme.textSecondary }]}
            >
                ...
                return (
                  ...
                  >
                    <View style={styles.speechTime}>
                      <ThemedText
                        style={[styles.speechTimeText, { color: theme.accent }]}
                      >
                        {speech.time || "TBA"}
                      </ThemedText>
                    </View>
                    ...
                    >
                      <ThemedText
                        style={[styles.speechBadgeText, { color: statusColor }]}
                      >
                        {statusText}
                      </ThemedText>
                    </View>
      ...
      {schedule.length === 0 ? (
        ...
        <View style={styles.emptyState}>
          <EvendiIcon name="search" size={28} color={theme.textSecondary} />
          <ThemedText
            style={[styles.emptyText, { color: theme.textSecondary }]}
          >
            Ingen treff i søket
        ...
        <Animated.View entering={FadeInDown.delay(400).duration(400)}>
          ...
          >
            <View style={styles.optimizationHeader}>
              <View style={styles.optimizationHeaderMain}>
                <EvendiIcon
                  name="alert-circle"
                  size={20}
                  color={Colors.dark.accent}
              ...
              <Pressable
                onPress={() => setShowOptimizationTips((prev) => !prev)}
                style={styles.optimizationToggle}
              >
                <EvendiIcon
                  name={showOptimizationTips ? "chevron-up" : "chevron-down"}
                  size={18}
                  color={Colors.dark.accent}
                />
              </Pressable>
            ...
            <ThemedText
              style={[
                styles.optimizationSubtext,
                { color: theme.textSecondary },
              ]}
            >
              {intervalsForUi.length} intervall
              {intervalsForUi.length > 1 ? "er" : ""} kan forbedres
            </ThemedText>

            {showOptimizationTips
              ? intervalsForUi.map((interval) => (
                  <Pressable
                    key={interval.id}
                    ...
                    <View style={styles.optimizationContent}>
                      ...
                      <View style={styles.timingInfo}>
                        <View style={styles.currentTime}>
                          ...
                          <ThemedText
                            style={[styles.timeBold, { color: theme.text }]}
                          >
      ...
      <Animated.View entering={FadeInDown.delay(600).duration(400)}>
        ...
        >
          ...
          <ThemedText
            style={[styles.tipsSubtitle, { color: theme.textSecondary }]}
          >
            Retningslinjer
          </ThemedText>
          ...
          <View style={styles.tipRow}>
            ...
            <View style={{ flex: 1 }}>
              <ThemedText style={[styles.tipText, { color: theme.text }]}>
                <ThemedText style={{ fontWeight: "600" }}>30+ min:</ThemedText>{" "}
                Ideell buffer
              </ThemedText>
              <ThemedText
                style={[styles.tipHint, { color: theme.textSecondary }]}
              >
      ...
      <View style={styles.addFabContainer}>
        <Pressable
          ...
          style={[styles.addFab, { backgroundColor: Colors.dark.accent }]}
        >
              ...
              <>
                ...
                <View style={styles.modalSection}>
                  ...
                  <PersistentTextInput
                    ...
                    style={[
                      styles.timeInput,
                      { borderColor: Colors.dark.accent, color: theme.text },
                    ]}
                    placeholder="14:30"
                    placeholderTextColor={theme.textSecondary}
                    value={newTime}
        ...
        >
          ...
          >
            ...
            <View style={styles.modalButtonGroup}>
              ...
              <Button onPress={handleCreateEvent} style={styles.saveButton}>
                Legg til
              </Button>
            </View>
          </View>
